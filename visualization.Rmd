...
おや...ヴァイオリンプロットは狙い通りの描画ができましたが，平均値を示す点は縦一列に並べられてしまっています。~~`geom_point()`のデフォルトの設定では，`position = "identity"`となっていたことを思い出してください。~~これは，`stat_summary()`のデフォルトの設定が，`position = "identity"`となっているためです。この設定を変える必要がありますね。  
【※2018/4/15　間違い修正】
